...
func ProcessPostbackEvent(
    ...
    if (dataSlice[0] == "NewReview" || dataSlice[0] == "AiReply") && dataSlice[1] == "GenerateAiReply" {
        ...
        err = handleGenerateAiReply(event.ReplyToken, user, businessId, reviewId, businessDao, reviewDao, line, log)
        if err != nil {
    ...
    case "RichMenu":
        ...
        case "QuickReplySettings":
            err := line.ShowQuickReplySettingsByBusinessGet(event.ReplyToken, user.ActiveBusinessId, businessDao)
            if err != nil {
        ...
        case "AiReplySettings":
            err := line.ShowAiReplySettingsByBusinessGet(event.ReplyToken, user, businessDao)
            if err != nil {
